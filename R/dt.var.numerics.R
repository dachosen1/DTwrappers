...
dt.var.numerics <- function(dat, the.variables = ".", na.rm = TRUE, the.filter = NULL, grouping.variables = NULL, grouping.type = "keyby", add.function.name = FALSE, non.numeric.value = "missing", ...){
  
  other.params <- sprintf("na.rm = %s", na.rm)
  
  return(dt.lapply(dat = dat, the.variables = the.variables, the.function = "var.numerics", the.filter = the.filter, grouping.variables = grouping.variables, grouping.type = grouping.type, add.function.name = add.function.name, non.numeric.value = non.numeric.value, other.params = other.params, ...))
}
