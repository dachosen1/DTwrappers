...
dt.remove.erroneous.characters <- function(dat, threshold.for.numeric = 0.5, the.variables = ".", the.filter = NULL, grouping.variables = NULL, grouping.type = "keyby", add.function.name = FALSE, ...){
  
  other.params <- sprintf("threshold.for.numeric = %s", threshold.for.numeric)
  
  return(dt.lapply(dat = dat, the.variables = the.variables, the.functions = "remove.erroneous.characters", the.filter = the.filter, grouping.variables = grouping.variables, add.function.name = add.function.name, other.params = other.params, ...))
}
