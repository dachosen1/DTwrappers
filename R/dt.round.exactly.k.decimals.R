...
dt.round.exactly.k.digits <- function(dat, k = 0, decimal = ".", the.variables = ".", the.filter = NULL, grouping.variables = NULL, grouping.type = "keyby", add.function.name = FALSE, ...){
  
  other.params <- sprintf("k = %d, decimal = %s", k, decimal)
  
  return(dt.lapply(dat = dat, the.variables = the.variables, the.functions = "round.exactly.k.digits", the.filter = the.filter, grouping.variables = grouping.variables, grouping.type = grouping.type, add.function.name = add.function.name, other.params = other.params, ...))
}
