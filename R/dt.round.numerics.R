...
dt.round.numerics <- function(dat, digits, the.variables = ".", the.filter = NULL, grouping.variables = NULL, grouping.type = "keyby", add.function.name = FALSE, ...){
  
  other.params <- sprintf("digits = %d", floor(digits))
  
  return(dt.lapply(dat = dat, the.variables = the.variables, the.functions = "round.numerics", the.filter = the.filter, grouping.variables = grouping.variables, grouping.type = grouping.type, add.function.name = add.function.name, other.params = other.params, ...))
}
