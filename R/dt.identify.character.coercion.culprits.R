...
#' @export
dt.identify.character.coercion.culprits <- function(dat, threshold.for.numeric = 0.5, the.variables = ".", the.filter = NULL, grouping.variables = NULL, grouping.type = "keyby", add.function.name = FALSE, ...){
  
  other.params <- sprintf("threshold.for.numeric = %s", threshold.for.numeric)
  
  return(dt.lapply(dat = dat, the.variables = the.variables, the.functions = "identify.character.coercion.culprits", the.filter = the.filter, grouping.variables = grouping.variables, grouping.type = grouping.type, add.function.name = add.function.name, other.params = other.params, ...))
}
