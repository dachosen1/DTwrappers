...
# Calculates the number of measured values for each specified variable in each group after applying a filter.

dt.total.measured <- function(dat, the.variables = ".", the.filter = NULL, grouping.variables = NULL, grouping.type = "keyby", add.function.name = FALSE, ...){
  
  return(dt.lapply(dat = dat, the.variables = the.variables, the.function = "total.measured", the.filter = the.filter, grouping.variables = grouping.variables, grouping.type = grouping.type, add.function.name = add.function.name, ...))
}
